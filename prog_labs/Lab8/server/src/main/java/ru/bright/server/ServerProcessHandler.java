    ...
    protected void handleRequest(SelectionKey key, UserRequest request)  {
        ...
        if (!(request.getCommandLine().trim().equalsIgnoreCase("auth") ||
            ...
            if (user != null && !server.getAuthManager().verify(user.getLogin(), user.getPassword())) {
                server.responseToClient(request, key, new Response(ResponseStatus.AUTH_FAILED, null));
                return;
            }
        }

        if (request.getAttachedObject() == null) {
            server.getServerCommandManager().executeCommand(request, key, request.getCommandLine());
        } else {
            ...
            try {
                ...
                server.responseToClient(request, key, new Response(ResponseStatus.ERROR, "Wrong command"));
                return;
            }
            ...
            if (cmd instanceof ServerObjectableCommand) {
                server.getServerCommandManager().executeObjectableCommand(request, key, request.getCommandLine(), request.getAttachedObject());
            }
        }
    }
}
