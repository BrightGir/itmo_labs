...
public class UpdateCommand extends ServerObjectableCommand {
    ...
    public boolean execute(User user,SelectionKey key, String[] args, Object object) {
        ...
        if(!toUpdateFlat.getOwnerLogin().equals(user.getLogin())) {
            getServer().sendError(key,("Элемент c ID " + id + " принадлежит пользователю " + toUpdateFlat.getOwnerLogin()));
        ...
        try {
            ...
            getServer().sendError(key, "Error while updating flat.");
